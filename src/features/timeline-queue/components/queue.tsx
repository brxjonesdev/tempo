...
export default function Queue({timeboxes, onSelectTimebox}: {
  timeboxes: TimeboxCardProps[];
  onSelectTimebox: (timebox: TimeboxCardProps) => void;
}) {
      ...
      <CardContent className="overflow-y-scroll p-4 space-y-4 flex-1 flex flex-col">
        ...
        .map((timebox, index) => (
          <TimeboxCard key={index} {...timebox}
              onSelect={() => onSelectTimebox(timebox)} 
           />
        ))}
        { timeboxes.length === 0 && (
